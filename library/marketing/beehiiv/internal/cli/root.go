...
type rootFlags struct {
	...
	// invocation. Mirrors the BEEHIIV_NO_LEARN env var.
	noLearn bool
...
Run 'beehiiv-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.selectFields, "select", "", "Comma-separated fields to include in output (e.g. --select advertisement_kind,advertiser_name,id)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'beehiiv-pp-cli profile list')")
	...
	rootCmd.AddCommand(newComplimentaryAccessCmd(flags))
